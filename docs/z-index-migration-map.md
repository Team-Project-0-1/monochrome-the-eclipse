...
### 전용 토큰 신설 (2026-05-30 세션, #7, 회귀 0)

기존 토큰에 값 일치 멤버가 없는 70/75/76은 **값 보존 + 역할 분리**를 위해 `tokens.css` §10에 전용 토큰 3개를 신설하고 1:1 매핑했다(값 이동 0). "방침상 진행하려면 새 토큰 추가 또는 육안 승인" 중 **새 토큰 추가** 경로의 첫 적용 사례.
...
검증: 브라우저 computed `getComputedStyle` 주입 측정 — 세 셀렉터 모두 76/75/70 정확 보존(pass). `--z-tooltip`(74)은 실제 툴팁 용도로 보존, 이 그룹에선 미사용.

누계: trivial(이전) + 값일치 21 + 전용토큰 신설 3 = 비-trivial 54 중 **24곳 전환**, **30곳 잔존**.
...
| 85 | 1 | `--z-coachmark`(80) | `.combat-active-skill-detail` 호버 팝오버. 토큰 80~92 사이 중간값 없음 → 80이면 `.combat-flash`(80)와 동률(현재 85>80), 92면 과상승 |

> ~~70, 75, 76~~ → #7에서 전용 토큰 신설로 해소 (위 ✅ 완료 참조). 보류 합계 33 → **30곳**.
