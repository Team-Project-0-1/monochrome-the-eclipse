...
### stage 강조 세분 토큰 (2026-05-30 세션, #13, 회귀 0)

raw 31/32/34(4곳)은 전부 combat-stage "30~49 강조" 밴드의 진짜 무대 요소라 모두 `--z-stage-*` 접두로 신설. 28<29<31<32<34<36 국소 적층이 의도된 순서라 banner(36)로 묶으면 평탄화+역전 → 값별 전용 토큰으로 보존. 34 둘(result-banner·assist-strip)은 역할이 달라(결과 배너 vs 보조 힌트) 20-분리 선례대로 역할별 2토큰.

| raw | 신설 토큰 (값) | 곳 | 셀렉터 |
|---|---|---|---|
| `31` | `var(--z-stage-foot-tray)` (31) | 1 | `.combat-foot-status-tray` 오버라이드(6851행) — base `--z-stage-overlay-fx`(28, 799행)를 덮어 31 지배 |
| `32` | `var(--z-stage-bottom-hud)` (32) | 1 | `.combat-bottom-hud` 하단 HUD 컨테이너 |
| `34` | `var(--z-stage-result-banner)` (34) | 1 | `.combat-result-banner` 전투 결과 배너 |
| `34` | `var(--z-stage-assist-strip)` (34) | 1 | `.combat-assist-strip` 보조 힌트 스트립 (result-banner와 값 공유, 다른 역할) |

검증: 브라우저 computed live — foot-status-tray **31**(그림자 31 규칙이 base 28을 덮어 효과값 지배, 보존 확인)·bottom-hud 32·result-banner 34·assist-strip 34 전부 보존(pass).

누계: 값일치 21 + 전용토큰 신설(#7·#9 4 + #8 10 + #10 6 + #11 5 + #12 4 + #13 4) = 비-trivial 54 중 **54곳 전환**, **0곳 잔존**. 🎉 P1-1 z-index 토큰화 전체 완료 — `components.css`에 raw 숫자 z-index 0개(전수 스캔 `z-index:\s*[0-9]` 확인).

## ✅ 보류 전부 해소 (2026-05-30, #13에서 마지막 그룹 처리)

당초 "값이 바뀌어야 해서 보류"했던 33곳은 모두 **값 이동 0**으로 처리됐다. 핵심 통찰: collapse가 위험했던 이유는 같은 밴드 내 1단위 차이가 **load-bearing**(의도된 국소 레이어링)이라서였고, 해법은 토큰 통일이 아니라 **값별 전용 토큰 신설**이었다. 마지막 31/32/34도 #13에서 같은 방식으로 해소.

> 해소 경로: ~~70, 75, 76~~ (#7), ~~85~~ (#9), ~~20, 26, 27, 28, 29~~ (#8), ~~2~~ (#10), ~~4, 5, 8~~ (#11), ~~10, 15, 18~~ (#12), ~~31, 32, 34~~ (#13) → 전부 전용 토큰 신설. 보류 합계 33 → **0곳**.

## 보류 해소 절차 (완료된 워크플로 기록)

1. 한 보류 그룹 선택 → 해당 raw 값 셀렉터를 grep으로 전부 확인.
2. 같은 그룹 내 셀렉터들이 화면에서 겹치는지·순서가 중요한지 판단.
   - 값 의미가 균일하면 단일 토큰 OK(#10 패턴).
   - 같은 값을 서로 다른 서브시스템/역할이 공유하면 **역할별 전용 토큰 분리**(#8·#11·#12·#13 패턴 — 값 우연 일치).
3. 치환 → `npm run build`(EXIT=0 + raw 잔존 grep 0) → 브라우저 `getComputedStyle` 주입으로 효과값 보존 검증 → 커밋(1 그룹 = 1 commit).
4. 처리 완료: ~~20대(20,26~29)~~ #8 · ~~`2`~~ #10 · ~~`4,5,8`~~ #11 · ~~`10,15,18`~~ #12 · ~~`31,32,34`~~ #13. **남은 보류 없음 — P1-1 전체 완료.**
